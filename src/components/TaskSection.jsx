...
export const TaskSection = ({ title, children }) => {
  return (
      <Disclosure as="div" className="w-full border-b border-slate-200">
        {({ open }) => (
          <>
            <DisclosureButton className="flex w-full px-1 py-3 text-left text-sm font-medium text-slate-700 hover:bg-slate-50">
              <ChevronRight className={`${open ? 'rotate-90' : ''} h-5 w-5 text-slate-500 transition-transform duration-200`}/>
              {title}
            </DisclosureButton>
            <DisclosurePanel className="text-sm text-slate-600">
              {children}
            </DisclosurePanel>
          </>
        )}
